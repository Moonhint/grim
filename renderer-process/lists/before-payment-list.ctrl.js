...
'use strict';
  ...
  function BeforePaymentListCtrl($scope, $mdDialog, $mdSidenav, Notification) {
    let self = this;
    self.sort_tracker = {};
    self.mass_compensation_ids = [];
    ...
    self.mass_compensation_select = mass_compensation_select;
    self.is_on_mass_selection = is_on_mass_selection;
    self.mass_compensation = mass_compensation;
    ...
    //events listener
    ipc.on('get-bills-response', function (event, arg) {
      if(arg.status === 200){
    ...
    ipc.on('create-bill-response', function (event, arg) {
      if(arg.status === 200){
        let query = {};
        if (self.filter_by === 'created_at' || self.filter_by === 'invoice_date' || self.filter_by === 'giro_due_date'){
        ...
        ipc.send('print-by-method', {
          bill: arg.data.bill,
          print_method: arg.data.print_method,
          fresh_created: true
        });
      }else if(arg.status === 400){
        Notification.error("Sinkronisasi gagal");
      }
    });
    ...
    function mass_compensation_select(bill){
      self.mass_compensation_ids = _.uniq(self.mass_compensation_ids);
      if (_.indexOf(self.mass_compensation_ids, bill._id) === -1){
        self.mass_compensation_ids.push(bill._id);
      }else{
        self.mass_compensation_ids = _.pull(self.mass_compensation_ids, bill._id);
      }
    }
    ...
    function mass_compensation(ev) {
      $mdDialog.show({
        controller: 'CompensateDialogCtrl',
        controllerAs: 'cdc',
        templateUrl: './renderer-process/lists/actions/compensate-dialog/dialog.tmpl.html',
        parent: angular.element(document.body),
        targetEvent: ev,
        clickOutsideToClose:true,
        fullscreen: false
      })
      .then(function(compensate_date) {
        ipc.send('compensate-bills', {
          bills: self.mass_compensation_ids,
          compensate_date: compensate_date
        });
      }, function() {
        console.info("cancel");
      });
    ...
    function edit(last_bill, ev) {
      let parent = self;
      $mdDialog.show({
        controller: 'EditBillDialogCtrl',
        controllerAs: 'ebdc',
        templateUrl: './renderer-process/lists/actions/edit-bill-dialog/dialog.tmpl.html',
        parent: angular.element(document.body),
        targetEvent: ev,
        clickOutsideToClose:true,
        fullscreen: false,
        locals: {last_bill: last_bill, current_user: self.current_user}
      })
      .then(function(edited_bill) {
        if(edited_bill.print_method){
          parent.bill_print = edited_bill.bill;
          parent.print_total = edited_bill.bill.dpp + edited_bill.bill.ppn + parseInt(edited_bill.bill.stamp);
          parent.print_today = moment().toDate();
          ipc.send('print-by-method', {
            bill: edited_bill.bill,
            print_method: edited_bill.print_method,
            fresh_created: false
          });
        }
        ipc.send('edit-bill', edited_bill.bill);
      }, function() {
        console.info("cancel");
      });
    }
  ...
  BeforePaymentListCtrl.$inject = ['$scope', '$mdDialog', '$mdSidenav', 'Notification'];
  angular.module('grimapp')
    .controller('BeforePaymentListCtrl', BeforePaymentListCtrl);
